/*!
 * @file    FMMKernelLaplace2LayerScalar.cpp
 * @author  Michal Merta 
...
void FMMKernelLaplace2Layer<LO, SC>::computeMoments( TreeMember<BECluster<LO, SC>* >* node ) {
  ...
  for ( int i = 0; i < momentArrSize; i++ ) {
    cluster->moments->momentsDL[i] = std::complex<SC>( 0.0, 0.0 );
  }
  ...
  if ( this->rightClusterTree->getNSons( node ) == 0 ) {
    ...
    for ( LO i = 0; i < cluster->nelems; i++ ) {
      ...
      this->beSpace->getMesh( )->getNormal( elemIdx, normal );
      ...
      for ( int j = 0; j < quad_size; j++ ) {
        ...
        for ( int n = 0; n <= nMax; n++ ) {
          for ( int m = 0; m <= n; m++ ) {
            if ( m == 0 ) {
              Rdx1 = half * ( getRnm( tempR, nMax, n - 1, -1 ) - getRnm( tempR, nMax, n - 1, 1 ) );
              Rdx2 = imagUnit * half * ( getRnm( tempR, nMax, n - 1, -1 ) + getRnm( tempR, nMax, n - 1, 1 ) );
              Rdx3 = getRnm( tempR, nMax, n - 1, 0 );
            } else {
              Rdx1 = half * ( getRnm( tempR, nMax, n - 1, m - 1 ) - getRnm( tempR, nMax, n - 1, m + 1 ) );
              Rdx2 = imagUnit * half * ( getRnm( tempR, nMax, n - 1, m - 1 ) + getRnm( tempR, nMax, n - 1, m + 1 ) );
              Rdx3 = getRnm( tempR, nMax, n - 1, m );
            }
            if ( m == 0 && n == 0 ) {
              Rdx1 = 0.0;
              Rdx2 = 0.0;
    ...
    for ( int i = 0; i < this->rightClusterTree->getNSons( node ); i++ ) {
      ...
      std::complex<SC> *sonMDL = this->rightClusterTree->get( node )->moments->momentsDL;
        ...
        for ( int k = n; k <= nMax; k++ ) {
          for ( int l = 0; l <= k; l++ ) {
            if ( parent->moments->localExpCoefDL ) {
              cluster->moments->localExpCoefDL[ j ] += ( getRnm( tempR, nMax, k - n, l - m ) ) *
