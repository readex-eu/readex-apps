/*!
 * @file    BEBilinearFormHelmholtz1Layer.cpp
 * @author  Jan Zapletal
 * @date    August 8, 2013
 * 
 */

#ifdef BEBILINEARFORMHELMHOLTZ1LAYER_H

namespace bem4i {

template<class LO, class SC>
BEBilinearFormHelmholtz1Layer<LO, SC>::BEBilinearFormHelmholtz1Layer( ) {
}

template<class LO, class SC>
BEBilinearFormHelmholtz1Layer<LO, SC>::BEBilinearFormHelmholtz1Layer(
    const BEBilinearFormHelmholtz1Layer& orig
    ) {
...
template <class LO, class SC>
void BEBilinearFormHelmholtz1Layer<LO, SC>::assemble(
...
template<class LO, class SC>
void BEBilinearFormHelmholtz1Layer<LO, SC>::assembleP0P0(
...
template<class LO, class SC>
void BEBilinearFormHelmholtz1Layer<LO, SC>::assembleP1DisP1Dis(
    const BECluster< LO, SC > * leftCluster,
    const BECluster< LO, SC > * rightCluster,
    FullMatrix< LO, SC > & matrix,
    void * voidIntegrator
...
template<class LO, class SC>
void BEBilinearFormHelmholtz1Layer<LO, SC>::assembleRow(
...
template<class LO, class SC>
void BEBilinearFormHelmholtz1Layer<LO, SC>::assembleRowP0P0(
    const BEBlockCluster< LO, SC > & block,
    LO idx,
    const std::vector<LO> & cols,
    Vector<LO, SC> & row,
...
template<class LO, class SC>
void BEBilinearFormHelmholtz1Layer<LO, SC>::assembleRowP1DisP1Dis(
    const BEBlockCluster< LO, SC > & block,
    LO idx,
    const std::vector<LO> & cols,
    Vector<LO, SC> & row,
...
template<class LO, class SC>
void BEBilinearFormHelmholtz1Layer<LO, SC>::assembleColumn(
    const BEBlockCluster< LO, SC > & block,
    LO idx,
...
template<class LO, class SC>
void BEBilinearFormHelmholtz1Layer<LO, SC>::assembleColumnP0P0(
    const BEBlockCluster< LO, SC > & block,
    LO idx,
    const std::vector<LO> & rows,
    Vector<LO, SC> & col,
...
template<class LO, class SC>
void BEBilinearFormHelmholtz1Layer<LO, SC>::assembleColumnP1DisP1Dis(
    const BEBlockCluster< LO, SC > & block,
    LO idx,
    const std::vector<LO> & rows,
    Vector<LO, SC> & col,
...
void BEBilinearFormHelmholtz1Layer<LO, SC>::assemble(
  ...
  //READEX_REGION_DEFINE(assemble_helmholtz_v);
  //READEX_REGION_START(assemble_helmholtz_v,"assemble_helmholtz_v",SCOREP_USER_REGION_TYPE_COMMON);
...
#ifdef VERBOSE
  ProgressMonitor::init( "Assembling Helmholtz V", iMax * jMax );
#endif
...
#pragma omp for collapse(2)
    for ( LO i = 0; i < iMax; i++ ) {
      //#pragma omp for
      for ( LO j = 0; j < jMax; j++ ) {
        integrator.getElemMatrix1Layer( outerElems[i], innerElems[j], *( matrixBuffer[counter] ) );
  ...
  //READEX_REGION_STOP(assemble_helmholtz_v); 
}
...
void BEBilinearFormHelmholtz1Layer<LO, SC>::assembleP0P0(
  ...
  //READEX_REGION_DEFINE(assemble_helmholtz_v00);
  //READEX_REGION_START(assemble_helmholtz_v00,"assemble_helmholtz_v00",SCOREP_USER_REGION_TYPE_COMMON);
...
#pragma omp for schedule( dynamic, 8 )
  ...
  //READEX_REGION_STOP(assemble_helmholtz_v00); 
}

template<class LO, class SC>
void BEBilinearFormHelmholtz1Layer<LO, SC>::assembleP1P1(
    FullMatrix<LO, SC> & matrix
    ) const {
