/*!
 * @file    FMMKernelLaplace1LayerScalarOMP.cpp
 * @author  Michal Merta 
...
#pragma omp critical
    {
      ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->tempR[omp_get_thread_num( )] = new std::complex<SC>[ (int) ( ( 2 * nMax + 2 )*( 2 * nMax + 1 ) / 2 )];
      ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->tmpM[omp_get_thread_num( )] = new std::complex<SC>[ (int) ( ( nMax + 2 )*( nMax + 1 ) / 2 )];
    }
  }
...
#pragma omp parallel
  {
    if ( ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->tempR[omp_get_thread_num( )] )
      delete [] ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->tempR[omp_get_thread_num( )];
    if ( ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->tmpM[omp_get_thread_num( )] )
      delete [] ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->tmpM[omp_get_thread_num( )];
  }
  if ( tempR )
    delete [] tempR;
  if ( tmpM )
...
void FMMKernelLaplace1Layer<LO, SC>::doUpwardPass( TreeMember<BECluster<LO, SC>* >* node ) {
  ...
  for ( int i = 0; i < ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->rightClusterTree->getNSons( node ); i++ ) {
    node = ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->rightClusterTree->getSonNode( node, i );
#pragma omp task
    doUpwardPass( node );
    node = ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->rightClusterTree->getParentNode( node );
  }

  // compute multipole moments
#pragma omp taskwait
  if ( ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->rightClusterTree->getNodeLevel( node ) >= ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->startLevel ) {
    computeMoments( node );
  }
}
...
void FMMKernelLaplace1Layer<LO, SC>::doDownwardPass( TreeMember<BECluster<LO, SC>* >* node ) {
  ...
  if ( ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->leftClusterTree->getNodeLevel( node ) >= ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->startLevel ) {
    computeLocExp( node );
  }

  // traverse the tree from the top
  for ( int i = 0; i < ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->leftClusterTree->getNSons( node ); i++ ) {


    node = ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->leftClusterTree->getSonNode( node, i );
#pragma omp task
    doDownwardPass( node );

    node = ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->leftClusterTree->getParentNode( node );
...
void FMMKernelLaplace1Layer<LO, SC>::computeMoments( TreeMember<BECluster<LO, SC>* >* node ) {
  ...
  FMMKernelLaplace1Layer<LO, SC>* thisClass = ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 );
  int nMax = thisClass->nMax;
  int quadOrder = thisClass->quadOrder;
  std::complex<SC> **tempR = ( ( FMMKernelLaplace1Layer<LO, SC>* )this_class_2 )->tempR;
  ...
  if ( thisClass->rightClusterTree->getNSons( node ) == 0 ) {
      ...
      for ( int j = 0; j < quad_size; j++ ) {
        ...
        if ( thisClass->beSpace->getAnsatzFunctionType( ) == p0 ) {
          for ( int k = 0; k < momentArrSize; k++ ) {
            cluster->moments->momentsSL[k] += ( tempR[omp_get_thread_num( )][k] * val[0] * ( (SC) quad_weight[j] ) * Jacob );
          }
    ...
    for ( int i = 0; i < thisClass->rightClusterTree->getNSons( node ); i++ ) {
      ...
      std::complex<SC> *sonMSL = thisClass->rightClusterTree->get( node )->moments->momentsSL;
        ...
        for ( int k = n; k <= nMax; k++ ) {
          for ( int l = 0; l <= k; l++ ) {
            if ( parent->moments->localExpCoefSL ) {
              cluster->moments->localExpCoefSL[ j ] += ( getRnm( tempR[omp_get_thread_num( )], nMax, k - n, l - m ) ) *
