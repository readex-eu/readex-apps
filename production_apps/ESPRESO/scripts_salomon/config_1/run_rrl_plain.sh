...
module purge
. env/readex_env/set_env_ptf_rapl.source
. env/readex_env/set_env_meric.source
. env/environment.sh

source env/paths.default
source env/threading.default 12

TMPFILE=espreso_rrl_plain.tmp
rm -f $TMPFILE
...
while [ $i -lt $REPEAT_COUNT ]; do
	...
	mpirun -n 2 ./espreso_ptf -c espreso.ecf | tee -a $TMPFILE
