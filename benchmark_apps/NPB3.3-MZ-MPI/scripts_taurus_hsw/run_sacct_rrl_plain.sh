...
#SBATCH -A p_readex
##SBATCH -J "bt_sacct_rrl"
##SBATCH --output=bt_sacct_rrl.out
##SBATCH --error=bt_sacct_rrl.out
...
while [ $i -le $REPEAT_COUNT ]; do
  ...
  srun  measure-rapl ./bin/bt-mz.C.2_ptf
  i=$(echo "$i + 1" | bc)
done
...
while [ $i -lt $REPEAT_COUNT ]; do
  ...
  echo "\n ================================================== \n "
  echo "times_energy= $times_energys \n"
  echo " ================================================== \n"
  i=$(echo "$i + 1" | bc)
  times_energys_array=(${times_energys[@]})
  time_step=${times_energys_array[7]}
  ...
  echo "\n total_energy_rrl= $total_energy_rrl \n "
  echo "\n ================================================== \n"
