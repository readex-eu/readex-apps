...
contains
  ...
  recursive subroutine  huti_scgsolv  ( ndim, wrkdim, xvec, rhsvec, &
    ...
    READEX_REGION_DEFINE(solver_iteration)
...
300 continue
    READEX_REGION_START(solver_iteration, 'solver_iteration', SCOREP_USER_REGION_TYPE_COMMON)
    ...
    if ( rho .eq. 0 ) then
       ...
       READEX_REGION_STOP(solver_iteration)
       write(*,*) "solver_iteration stop"
       go to 1000
    end if
