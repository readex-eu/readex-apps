...
!------------------------------------------------------------------------------
    ...
    READEX_REGION_DEFINE(ElmerSolver_core)
    READEX_REGION_DEFINE(read_mesh)
    READEX_REGION_DEFINE(add_solvers)
    READEX_REGION_DEFINE(simulation)
    READEX_REGION_DEFINE(exec_simulation)
#ifdef HAVE_TRILINOS
INTERFACE
...
END INTERFACE
    ...
    READEX_INIT()
    READEX_PHASE_START(ElmerSolver_core, 'ElmerSolver_core', SCOREP_USER_REGION_TYPE_COMMON)
...
!------------------------------------------------------------------------------
     MeshIndex = 0
     DO WHILE( .TRUE. )
       READEX_REGION_START(read_mesh, 'read_mesh', SCOREP_USER_REGION_TYPE_COMMON)
       ...
       ELSE
         ...
         IF ( .NOT.ReloadInputFile(CurrentModel) ) THEN
            READEX_REGION_STOP(read_mesh)
            EXIT
         END IF
...
!      memory for the dofs
!------------------------------------------------------------------------------
       READEX_REGION_START(add_solvers, 'add_solvers', SCOREP_USER_REGION_TYPE_COMMON)
       CALL AddSolvers()
       READEX_REGION_STOP(add_solvers)
...
!------------------------------------------------------------------------------
       ...
       READEX_REGION_STOP(read_mesh)
       READEX_REGION_START(simulation, 'simulation', SCOREP_USER_REGION_TYPE_COMMON)
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
       ...
       ELSE
          READEX_REGION_START(exec_simulation, 'exec_simulation', SCOREP_USER_REGION_TYPE_COMMON)
          CALL ExecSimulation( TimeIntervals, CoupledMinIter, &
              CoupledMaxIter, OutputIntervals, Transient, Scanning)
          READEX_REGION_STOP(exec_simulation)
       END IF
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
     ...
     READEX_PHASE_STOP(ElmerSolver_core)
     READEX_CLOSE()
#ifdef USE_SCOREP
