...
!------------------------------------------------------------------------------
  ...
  READEX_REGION_DEFINE(create_instance)
  READEX_REGION_DEFINE(call_solver)
...
!  END INTERFACE
  ...
  IF ( Factorize .OR. .NOT.C_ASSOCIATED(A % PermonSolverInstance) ) THEN
    ...
    READEX_REGION_START(create_instance, 'create_instance', SCOREP_USER_REGION_TYPE_COMMON)
    ...
    READEX_REGION_STOP(create_instance)

  READEX_REGION_START(call_solver, 'call_solver', SCOREP_USER_REGION_TYPE_COMMON)
  !CALL Permon_Solve( A % PermonSolverInstance, x, b )
  CALL FETI4ISolve(A % PermonSolverInstance, n_dof_partition, x)
  READEX_REGION_STOP(call_solver)
#else
