...
MODULE IterSolve
   ...
   READEX_REGION_DEFINE(iter_solver_call)
...
!------------------------------------------------------------------------------
    ...
    IF ( ComplexSystem ) THEN
      ...
      READEX_REGION_START(iter_solver_call, 'iter_solver_call', SCOREP_USER_REGION_TYPE_COMMON)
      CALL IterCall( iterProc, xC, bC, ipar, dpar, workC, &
          mvProc, pcondProc, pcondrProc, dotProc, normProc, stopcProc )
      READEX_REGION_STOP(iter_solver_call)
