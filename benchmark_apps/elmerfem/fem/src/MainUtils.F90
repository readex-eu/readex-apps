...
!------------------------------------------------------------------------------
    ...
    READEX_REGION_DEFINE(solver_activate)
!------------------------------------------------------------------------------
    ...
    IF( ExecSlot ) THEN
      ...
      DO k=1,nSolvers
        ...
        IF ( Solver % SolverExecWhen == SOLVER_EXEC_AHEAD_TIME .OR. &
          ...
          READEX_REGION_START(solver_activate, 'solver_activate', SCOREP_USER_REGION_TYPE_COMMON) 
          CALL SolverActivate( Model,Solver,dt,TransientSimulation )
          READEX_REGION_STOP(solver_activate)
