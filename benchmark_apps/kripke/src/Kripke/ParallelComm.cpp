...
#ifdef USE_SCOREP
READEX_REGION_DEFINE(postRecvs);
READEX_REGION_START(postRecvs, "postRecvs", SCOREP_USER_REGION_TYPE_COMMON);
#endif
...
#ifdef USE_SCOREP
READEX_REGION_STOP(postRecvs);
#endif
}

void ParallelComm::postSends(Subdomain *sdom, double *src_buffers[3]){
#ifdef USE_SCOREP
READEX_REGION_DEFINE(postSends);
READEX_REGION_START(postSends, "postSends", SCOREP_USER_REGION_TYPE_COMMON);
#endif
