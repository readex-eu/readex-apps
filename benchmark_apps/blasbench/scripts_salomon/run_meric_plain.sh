...
#PBS -q qprod
##PBS -q qexp
#PBS -N blasbench_meric_plain
#PBS -o blasbench_meric_plain.out
#PBS -e blasbench_meric_plain.err
...
TMPFILE=blasbench_meric_plain.tmp
rm -f $TMPFILE
...
while [ $i -lt $REPEAT_COUNT ]; do
	...
	mpirun $BIND_TO_SOCKETS ./blasbench_plain --dgemv 300,8192 --dgemm 150,2048 --tan 60000 --phase 2 2>&1 | tee -a $TMPFILE
...
while [ $i -lt $REPEAT_COUNT ]; do
	$MERIC_ROOT/tools/staticMERICtool/multiNodeStaticMeasureStart.sh --rapl
	mpirun $BIND_TO_SOCKETS ./blasbench_meric --dgemv 300,8192 --dgemm 150,2048 --tan 60000 --phase 2 2>&1 | tee -a $TMPFILE
	OUTPUT=$($MERIC_ROOT/tools/staticMERICtool/multiNodeStaticMeasureStop.sh --rapl)
	Evaluate
	i=$[ $i + 1 ]
done
