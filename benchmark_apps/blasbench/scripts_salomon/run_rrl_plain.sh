...
#PBS -q qprod
##PBS -q qexp
#PBS -N blasbench_rrl_plain
#PBS -o blasbench_rrl_plain.out
#PBS -e blasbench_rrl_plain.err
...
TMPFILE=blasbench_rrl_plain.tmp
rm -f $TMPFILE
...
while [ $i -lt $REPEAT_COUNT ]; do
	$MERIC_ROOT/tools/staticMERICtool/multiNodeStaticMeasureStart.sh --rapl
	mpirun $BIND_TO_SOCKETS ./blasbench_ptf --dgemv 300,8192 --dgemm 150,2048 --tan 60000 --phase 2 2>&1 | tee -a $TMPFILE
	OUTPUT=$($MERIC_ROOT/tools/staticMERICtool/multiNodeStaticMeasureStop.sh --rapl)
	Evaluate
	i=$[ $i + 1 ]
done
