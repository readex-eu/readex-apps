...
{
	std::stringstream ss;
	std::vector<char> tmp (60,'-');

	for( std::vector<char>::const_iterator i = tmp.begin(); i != tmp.end(); ++i) {
		ss << *i << ' ';
	}
	ss << "\n";
...
void SparseMatrix::Clear() {
	...
	std::vector<eslocal>().swap( I_row_indices );
	std::vector<eslocal>().swap( J_col_indices );
	std::vector<double>().swap( V_values );

	std::vector<eslocal>().swap( CSR_I_row_indices );
	std::vector<eslocal>().swap( CSR_J_col_indices );
	std::vector<double>().swap( CSR_V_values );

	std::vector<double>().swap( dense_values );
	std::vector<float>().swap( dense_values_fl );
...
eslocal  SparseMatrix::SaveMatrixBinInCOO(string filename) {
	...
	if ( out.is_open() ) {
		...
		std::cout << "Matrix file " << filename << " cannot be created ! ";
		return -1;
	}
...
eslocal SparseMatrix::LoadMatrixBinInCOO(string filename, char matrix_type_G_for_general_S_for_symmetric) {
	...
	if ( in.is_open() ) {
		...
		std::cout << "Matrix file " << filename << " not found ! ";
		return -1;
...
//	dense_values_fl   = A_in.dense_values_fl;
	...
	std::cout << "Matrix " << Matname << " sizes:";
	std::cout << "DNS size: " << dense_size << " B";
	std::cout << "CSR size: " << CSR_size << " B";
	std::cout << "IJV size: " << IJV_size << " B";
}
...
void SparseMatrix::DenseMatVec(std::vector <double> & x_in, std::vector <double> & y_out) {
	DenseMatVec(x_in, y_out, 'N', 0, 0, 0.0);
}


void SparseMatrix::DenseMatVec(std::vector <double> & x_in, std::vector <double> & y_out, char T_for_transpose_N_for_not_transpose ) {
	DenseMatVec(x_in, y_out, T_for_transpose_N_for_not_transpose, 0, 0, 0.0);
}

void SparseMatrix::DenseMatVec(std::vector <double> & x_in, std::vector <double> & y_out, char T_for_transpose_N_for_not_transpose, eslocal x_in_vector_start_index) {
	DenseMatVec(x_in, y_out, T_for_transpose_N_for_not_transpose, x_in_vector_start_index, 0, 0.0);
}

void SparseMatrix::DenseMatVec(std::vector <double> & x_in, std::vector <double> & y_out, char T_for_transpose_N_for_not_transpose, eslocal x_in_vector_start_index, eslocal y_out_vector_start_index) {
	DenseMatVec(x_in, y_out, T_for_transpose_N_for_not_transpose, x_in_vector_start_index, y_out_vector_start_index, 0.0);
}

void SparseMatrix::DenseMatVec(std::vector <double> & x_in, std::vector <double> & y_out, char T_for_transpose_N_for_not_transpose, eslocal x_in_vector_start_index, eslocal y_out_vector_start_index, double beta) {
	...
	if (type == 'S') {
		if ( T_for_transpose_N_for_not_transpose == 'T' ) {
                    std::cout << "Transposition is not supported for packed symmetric matrices";
                    return;
		} else {
...
void SparseMatrix::DenseMatMat(SparseMatrix & A_in, SparseMatrix & B_in, char T_for_transpose_N_for_non_transpose) {
...
void SparseMatrix::DenseMatVecCUDA_w_Copy(std::vector <double> & x_in, std::vector <double> & y_out, char T_for_transpose_N_for_not_transpose, eslocal x_in_vector_start_index) {
#ifdef CUDA
...
void SparseMatrix::DenseMatVecCUDA_wo_Copy(std::vector <double> & x_in, std::vector <double> & y_out, char T_for_transpose_N_for_not_transpose, eslocal x_in_vector_start_index) {
#ifdef CUDA
...
//		std::cout << "Error during host to device copy";
//		MPI_Finalize();
//		exit(0);
//	}
//
...
//		std::cout << "Error during cublas DenseMatVec";
//		MPI_Finalize();
//		exit(0);
//	}
//
...
//	if (status != cudaSuccess)   {
//		std::cout << "Error during device to host copy";
//		MPI_Finalize();
//		exit(0);
//	}
//
...
void SparseMatrix::DenseMatVecCUDA_shared_wo_Copy_start( double * x_in, double * y_out, char T_for_transpose_N_for_not_transpose, eslocal x_in_vector_start_index) {
	...
	if (status != cudaSuccess)   {
		std::cout << "Error " << cudaGetErrorString(status) << " during host to device copy";
		MPI_Finalize();
		exit(0);
	}
...
//		}
	...
	if (cublas_status != CUBLAS_STATUS_SUCCESS)   {
		std::cout << "Error " << _cudaGetErrorEnum(cublas_status) << " during cublas DenseMatVec";
		MPI_Finalize();
		exit(0);
	}
...
eslocal SparseMatrix::MallocOnCUDA_Dev ( ) {
	...
	if (dense_values.size() > 0) {
		...
		if (rows > 0 && cols > 0) {
			...
			std::cout << "GPU ERROR - Matrix on GPU cannot be allocated - no valid size of rows or cols \n";
			exit(0);
		}
	}
	...
	if (status != cudaSuccess) {
		//std::cout << "Error allocating GPU memory for input vector";
		//MPI_Finalize();
		//exit(0);
		error = -1;
		}
	...
	if (status != cudaSuccess) {
		//std::cout << "Error allocating GPU memory for output vector";
		//MPI_Finalize();
		//exit(0);
		error = -1;
	}
...
//						LAPACK_ROW_MAJOR
	...
	if (USE_FLOAT) {
		...
		std::vector <double> tmp_dense ( (rows *(rows +1))/2, 0.0 ) ;
...
void SparseMatrix::MatVecCOO(std::vector <double> & x_in, std::vector <double> & y_out ) {
	char T_for_transpose_N_for_non_transpose = 'N';
	MatVecCOO(x_in, y_out, T_for_transpose_N_for_non_transpose);
}

void SparseMatrix::MatVecCOO(std::vector <double> & x_in, std::vector <double> & y_out, char T_for_transpose_N_for_non_transpose) {
	double beta = 0.0;
	MatVecCOO(x_in, y_out, T_for_transpose_N_for_non_transpose, beta);
}

void SparseMatrix::MatVecCOO(std::vector <double> & x_in, std::vector <double> & y_out, char T_for_transpose_N_for_non_transpose, double beta) {
	double alpha = 0.0;
	MatVecCOO(x_in, y_out, T_for_transpose_N_for_non_transpose, beta, alpha);
}

void SparseMatrix::MatVecCOO(std::vector <double> & x_in, std::vector <double> & y_out, char T_for_transpose_N_for_non_transpose, double beta, double alpha) {
...
void SparseMatrix::MatVec(std::vector <double> & x_in, std::vector <double> & y_out, char T_for_transpose_N_for_non_transpose ) {
	MatVec(x_in, y_out, T_for_transpose_N_for_non_transpose, 0 , 0);
}

void SparseMatrix::MatVec(std::vector <double> & x_in, std::vector <double> & y_out, char T_for_transpose_N_for_non_transpose, eslocal x_in_vector_start_index, eslocal y_out_vector_start_index) {
	double beta = 0;
	MatVec(x_in, y_out, T_for_transpose_N_for_non_transpose, x_in_vector_start_index, y_out_vector_start_index, beta);
}

void SparseMatrix::MatVec(std::vector <double> & x_in, std::vector <double> & y_out, char T_for_transpose_N_for_non_transpose, eslocal x_in_vector_start_index, eslocal y_out_vector_start_index, double beta) {
	// y := alpha*A*x + beta*y
	// or
	// y := alpha*A'*x + beta*y
...
void SparseMatrix::printMatCSR(char *str0){
	...
	for (eslocal i = 0; i < rows; i++) {
		for (eslocal j = CSR_I_row_indices[i]; j < CSR_I_row_indices[i + 1]; j++) {
			std::cout << i + 1 << " " << CSR_J_col_indices[j - offset] << " " << CSR_V_values[j - offset];
		}
	}
	std::cout << "];" << str0 << " = full(sparse(" << str0 << "(:,1)," << str0 << "(:,2)," << str0 << "(:,3)," << rows << "," << cols << "));";
	if (type=='S'){
		std::cout << str0 << "=" << str0 << "+" << str0 << "'-diag(diag(" << str0 << "));";
	}
}
...
void SparseMatrix::getNullPivots(std::vector <eslocal> & null_pivots){
#ifdef GENINVtools
	std::vector <double> N(dense_values);
...
//
  ...
  estim_cond=fabs(alphaVec[cnt-1]/alphaVec[0]);
	if (plot_n_first_n_last_eigenvalues > 0) {
		std::cout << "conds(" << str0 << ") = " << estim_cond << "\tit: " << cnt;
	}
...
void SparseMatrix::MatAddInPlace(SparseMatrix & B_in, char MatB_T_for_transpose_N_for_non_transpose, double beta) {
	...
	if (!B_in.nnz == 0) {
		...
		if (nnz == 0 && transa == 'T' && beta == 1.0) {
			std::cout << "Error in 'SparseMatrix::MatAddInPlace' - not implemented - " << "beta = " << beta << " Trans = " << transa;
			return;
		}
		...
		std::vector<eslocal>		t_CSR_I_row_indices;	t_CSR_I_row_indices.resize( m + 1 );
		std::vector<eslocal>		t_CSR_J_col_indices;	t_CSR_J_col_indices.resize(1);
		std::vector<double>		t_CSR_V_values;			t_CSR_V_values.resize(1);
...
void SparseMatrix::MatTranspose(SparseMatrix & A_out) {
	...
	std::vector<eslocal>().swap( A_out.CSR_I_row_indices );
	std::vector<eslocal>().swap( A_out.CSR_J_col_indices );
	std::vector<double>().swap( A_out.CSR_V_values );
...
void SparseMatrix::MatTranspose() {
	...
	std::vector <eslocal> tCSR_I_row_indices;
	std::vector <eslocal> tCSR_J_col_indices;
	std::vector <double> tCSR_V_values;
...
void SparseMatrix::RemoveLower() {
	...
	std::vector <eslocal> t_CSR_I_row_indices;
	std::vector <eslocal> t_CSR_J_col_indices;
	std::vector <double> t_CSR_V_values;
	eslocal l_nnz = 0;
	...
	case MatrixType::REAL_SYMMETRIC_INDEFINITE:
		break;
	default:
		std::cout << "Unknown mtype in remove lower";
	}
...
void SparseMatrix::CreateMatFromRowsFromMatrix(SparseMatrix & A_in, std::vector <eslocal> & rows_to_add) {
...
void SparseMatrix::CreateMatFromRowsFromMatrix_NewSize(SparseMatrix & A_in, std::vector <eslocal> & rows_to_add) {
...
void SparseMatrix::MatMatT(SparseMatrix & A_in, SparseMatrix & B_in) {
	...
	//std::vector < std::vector < eslocal    > > GGt_J (A_in.rows, std::vector < eslocal    > () );
	//std::vector < std::vector < double > > GGt_V (A_in.rows, std::vector < double > () );
...
//  - OWN
  ...
  if (!use_null_pivots_or_s_set) diagonalRegularization=false;
    ...
    std::string name;
    if (d_sub==-1){
      name = "kernel_detct_GGt";
    }
    else{
      name = std::to_string(d_sub) + "kernel_detct";
    }
  ...
  std::vector <eslocal > permVec;

  permVec.resize(K.rows);
  std::vector <std::vector<eslocal >> vec_I1_i2(K.rows, std::vector<eslocal >(2, 1));
  ...
  std::vector <eslocal > tmp_vec_s;
  tmp_vec_s.resize(sc_size);
  eslocal v1, n_mv, cnt_permut_vec;
  std::vector <eslocal >::iterator it;
  std::vector <eslocal > fix_dofs;
  ...
  std::vector <double> tmp_approx_max_eig;
...
#endif
    if (info){
      std::cout << "info = " << info << " something wrong with Schur complement in SparseSolverMKL::generalIinverse";
    }
    delete [] WK_modif;
    delete [] ZK_modif;
  }
...
//
      ...
      std::sort(vec_I1_i2.begin(), vec_I1_i2.end(),
                [](const std::vector <eslocal >& a, const std::vector <eslocal >& b) {
        return a[0] < b[0];
      });
  ...
  else
  {
    if (SC_via_K_rr){
      ...
      if (error_K_rr){
        // K  ------------------------------------------------------------------------
        SparseMatrix se0 = K;
        std::ofstream ose00(std::to_string(d_sub) + "K_Err");
        ose00 << se0;
        ose00.close();
        // K_rr ----------------------------------------------------------------------
        SparseMatrix se1 = K_rr;
        std::ofstream ose0(std::to_string(d_sub) + "K_rrErr");
        ose0 << se1;
        ose0.close();
        // K_rs ----------------------------------------------------------------------
        SparseMatrix se2 = K_rs;
        std::ofstream ose1(std::to_string(d_sub) + "K_rsErr");
        ose1 << se2;
        ose1.close();
        // K_modif -------------------------------------------------------------------
        SparseMatrix se3 = K_modif;
        std::ofstream ose2(std::to_string(d_sub) + "K_modifErr");
        ose2 << se3;
        ose2.close();
        // info file -----------------------------------------------------------------
        if (d_sub!=-1){
          std::ofstream ose3(std::to_string(d_sub) + "permut_vectorErr");
          for (size_t i = 0;i<permVec.size();i++){
            ose3 << permVec[i]+1 <<" ";
          }
          ose3.close();
        ...
        os.close();
#endif
        std::cout << "factorization of K_rr failed (1/2 factorization).";
        exit(EXIT_FAILURE);
      }
...
// EIGENVALUES AND EIGENVECTORS OF SCHUR COMPLEMENT
  ...
  if (info){
    std::cout <<"info = " << info << " something wrong with Schur complement in SparseSolverMKLCPU::generalIinverse";
  }
#if VERBOSE_KERNEL>0
...
//
  ...
  std::vector <eslocal > null_pivots;
...
//    os.close();
//#endif
//    std::cout << "factorization of Kreg failed (2/2 factorization).";
//    exit(EXIT_FAILURE);
//  }
//  ///////////////////////////////////////////////////////////////////////////////////
//
...
//0 - allocation of vectors, copying of matrix
  ...
  //K.printMatCSR("K");
  std::vector <double> diagonals = K_modif.getDiagonal();
...
//
  ...
  if (K_rr_cols!=0){
//    SparseSolverMKLCPU K_rr_t_solver;
//    SparseMatrix K_rr_t;
...
//
  ...
  std::vector <eslocal > null_pivotsl;
