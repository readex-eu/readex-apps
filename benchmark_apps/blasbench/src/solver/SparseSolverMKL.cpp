...
void SparseSolverMKL::SetThreaded() {
	...
	/* Numbers of processors, value of OMP_NUM_THREADS */
	int num_procs = 1; //std::stoi(getenv("SOLVER_NUM_THREADS"));
    iparm[2]  = num_procs;
}
...
int SparseSolverMKL::Factorization(const std::string &str) {
    ...
    return error;
		...
		std::ofstream osK("ERROR");
		osK << s;
		osK.close();

		std::cout << error << " during symbolic factorization";
		exit (EXIT_FAILURE);
	} else {
		initialized = true;
	}
	...
	if (error != 0)
	{
		...
		std::cout << error << " during numerical factorization";
		exit (EXIT_FAILURE);
	} else {
		m_factorized = 1;
	}
...
void SparseSolverMKL::Solve( std::vector <double> & rhs_sol) {
	...
	if (error != 0)
	{
		...
		std::cout << "ERROR during solution: " << error;
		exit (3);
	}
...
void SparseSolverMKL::Solve( std::vector <double> & rhs, std::vector <double> & sol, MKL_INT n_rhs) {
...
void SparseSolverMKL::Solve( std::vector <double> & rhs, std::vector <double> & sol, MKL_INT rhs_start_index, MKL_INT sol_start_index) {
...
void SparseSolverMKL::SolveMatF( espreso::SparseMatrix & A_in, espreso::SparseMatrix & B_out, bool isThreaded ) {
	...
	std::vector<MKL_INT> perm (A_in.dense_values.size() , 0);
	...
	if (error != 0)
	{
		...
		std::cout << "ERROR during the solution of the system: " << error;
		exit (1);
	} else {
		initialized = true;
	}
	...
	//std::vector<double>().swap( A_in.dense_values );
...
//		s.type = 'S';
		...
		std::cout << "ERROR during numerical factorization: " << error;
		exit (2);
	} else {
		initialized = true;
	}
...
void SparseSolverMKL::Create_SC_w_Mat( espreso::SparseMatrix & K_in, espreso::SparseMatrix & B_in, espreso::SparseMatrix & SC_out,
	...
	if (isThreaded) {
		/* Numbers of processors, value of OMP_NUM_THREADS */
		MKL_INT num_procs = 1; //std::stoi(getenv("SOLVER_NUM_THREADS"));
	    iparm[2] = num_procs;
	} else {
		iparm[2] = 1;
	}
...
void SparseSolverMKL::SolveCG(espreso::SparseMatrix & A_in, std::vector <double> & rhs_sol) {
...
void SparseSolverMKL::SolveCG(espreso::SparseMatrix & A_in, std::vector <double> & rhs_in, std::vector <double> & sol) {
	std::vector<double> init;
	SolveCG(A_in, rhs_in, sol, init);
}

void SparseSolverMKL::SolveCG(espreso::SparseMatrix & A_in, std::vector <double> & rhs_in, std::vector <double> & sol, std::vector <double> & initial_guess) {
...
//	  double  * a   = CSR_V_values;
	  ...
	  std::vector <double> tmp_vec (4 * n, 0);
	  double * tmp;
	  tmp = &tmp_vec[0];
