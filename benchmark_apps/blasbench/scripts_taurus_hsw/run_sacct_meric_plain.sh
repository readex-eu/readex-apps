...
#SBATCH -J "blasbench_meric_sacct"
#SBATCH -A p_readex
#SBATCH --reservation=READEX
#SBATCH --output=blasbench_meric_sacct.out
#SBATCH --error=blasbench_meric_sacct.out
...
while [ $i -le $REPEAT_COUNT ]; do
  ...
  srun --cpu_bind=verbose,sockets measure-rapl ./blasbench_plain --dgemv 300,8192 --dgemm 150,2048 --tan 60000 --phase 10
  i=$(echo "$i + 1" | bc)
done
...
while [ $i -le $REPEAT_COUNT ]; do
  ...
  srun --cpu_bind=verbose,sockets measure-rapl ./blasbench_meric --dgemv 300,8192 --dgemm 150,2048 --tan 60000 --phase 10
  i=$(echo "$i + 1" | bc)
done
