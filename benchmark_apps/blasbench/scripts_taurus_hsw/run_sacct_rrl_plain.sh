...
#SBATCH -J "blasbench_sacct"
#SBATCH -A p_readex
#SBATCH --reservation=READEX
#SBATCH --output=blasbench_sacct.out
#SBATCH --error=blasbench_sacct.out
...
while [ $i -le $REPEAT_COUNT ]; do
  ...
  srun --cpu_bind=verbose,sockets measure-rapl ./blasbench_ptf --dgemv 300,8192 --dgemm 150,2048 --tan 60000 --phase 10
  i=$(echo "$i + 1" | bc)
done
