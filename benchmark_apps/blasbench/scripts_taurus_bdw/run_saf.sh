...
while [ $result != 0 ]; do
  echo "result = "$result
  srun --cpu_bind=verbose,sockets --nodes 1 --ntasks-per-node 2 --cpus-per-task 14 ./blasbench_saf --dgemv 300,8192 --dgemm 150,2048 --tan 50000 --phase 2
...
if [ "$READEX_INTEL" == "1" ]; then
  echo "^\/.*include.*bits\/.*\.h:.*$ off" >> scorep_icc.filt
  echo "^\/.*include.*bits\/.*\.hpp:.*$ off" >> scorep_icc.filt
  echo "^\/.*include.*bits\/.*\.H:.*$ off" >> scorep_icc.filt
fi
