...
CORE_MAX_FREQ="2500"
CORE_FREQ_STEP="400"
CORE_FREQ_DEFAULT="2500"

# value "" if not used
UNCORE_MIN_FREQ="1400"
UNCORE_MAX_FREQ="3000"
UNCORE_FREQ_STEP="400"
UNCORE_FREQ_DEFAULT="3000"

# value "" if not used
OMPT_LOWER_VALUE="12" #"24"
OMPT_STEP="4" #"24"
...
# if "" then ATP tags are omitted, else default is "exhaustive_atp"; keep 1 for individual
# ATP_SEARCH_ALGORITHM="exhaustive_atp"
ATP_SEARCH_ALGORITHM="" #"individual_atp"
...
ptf_config_file="readex_config_ptf.xml" # extended readex_config file for PTF
rm -f $ptf_config_file
...
IFS=' ' read -r -a algo <<< "$SEARCH_ALGORITHM"
  ...
  if [ "$CORE_MIN_FREQ" != "" ] && [ "$CORE_MAX_FREQ" != "" ] && [ "$CORE_FREQ_STEP" != "" ] && [ "$CORE_FREQ_DEFAULT" != "" ]; then
    ...
    echo "            <default>$CORE_FREQ_DEFAULT</default>" >> $ptf_config_file
    echo "        </frequency>" >> $ptf_config_file
  fi
  if [ "$UNCORE_MIN_FREQ" != "" ] && [ "$UNCORE_MAX_FREQ" != "" ] && [ "$UNCORE_FREQ_STEP" != "" ] && [ "$UNCORE_FREQ_DEFAULT" != "" ]; then
    ...
    echo "            <default>$UNCORE_FREQ_DEFAULT</default>" >> $ptf_config_file
    echo "        </uncore>" >> $ptf_config_file
  fi
  ...
  echo "        </searchAlgorithm>" >> $ptf_config_file
  if [ "$ATP_SEARCH_ALGORITHM" != "" ]; then
    echo "        <atp>" >> $ptf_config_file
    echo "            <searchAlgorithm>" >> $ptf_config_file
    if [ "$ATP_SEARCH_ALGORITHM" = "exhaustive_atp" ]; then
      echo "                <name>exhaustive_atp</name>" >> $ptf_config_file
    elif [ "$ATP_SEARCH_ALGORITHM" = "individual_atp" ]; then
      echo "                <name>individual_atp</name>" >> $ptf_config_file
      echo "                    <keep>1</keep>" >> $ptf_config_file
    else
      echo "                <name>exhaustive_atp</name>" >> $ptf_config_file
    fi
    echo "            </searchAlgorithm>" >> $ptf_config_file
  ...
  echo "        <tuningModel>" >> $ptf_config_file
  echo "            <file_path>$TUNING_MODEL_PATH</file_path>" >> $ptf_config_file
  echo "        </tuningModel>" >> $ptf_config_file
  echo "    </periscope>" >> $ptf_config_file
