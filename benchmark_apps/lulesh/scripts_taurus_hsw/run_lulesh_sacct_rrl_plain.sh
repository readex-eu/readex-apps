...
#SBATCH -J "lulesh_sacct_intel"
#SBATCH -A p_readex
#SBATCH --reservation=READEX
#SBATCH --output=lulesh_1_node.out
#SBATCH --error=lulesh_1_node.out
...
while [ $i -le $REPEAT_COUNT ]; do
  ...
  #srun --cpu_bind=verbose,sockets measure-rapl ./lulesh2.0_plain -i 200 -s 75 
...
while [ $i -le $REPEAT_COUNT ]; do
  ...
  srun --ntasks 1 --ntasks-per-node 1 --cpus-per-task 24 measure-rapl ./lulesh2.0_ptf -i 100 -s 150 
  i=$(echo "$i + 1" | bc)
done
