...
if [ -e $rdd_config_file ]; then
  ...
  echo "            <min_freq>2400000</min_freq>" >> $ptf_config_file
  echo "            <max_freq>2400000</max_freq>" >> $ptf_config_file
  echo "            <freq_step>2400000</freq_step>" >> $ptf_config_file
  echo "        </frequency>" >> $ptf_config_file
  echo "        <uncore>" >> $ptf_config_file
  echo "            <min_freq>24</min_freq>" >> $ptf_config_file
  echo "            <max_freq>24</max_freq>" >> $ptf_config_file
  echo "            <freq_step>24</freq_step>" >> $ptf_config_file
  echo "        </uncore>" >> $ptf_config_file
  echo "        <openMPThreads>" >> $ptf_config_file
  echo "            <lower_value>12</lower_value>" >> $ptf_config_file
  echo "            <step>12</step>" >> $ptf_config_file
  echo "        </openMPThreads>" >> $ptf_config_file
  echo "    </tuningParameter>" >> $ptf_config_file
...
#  echo "        </atp>" >> $ptf_config_file
  ...
  echo "            <file_path>tuning_model.json</file_path>" >> $ptf_config_file
  echo "        </tuningModel>" >> $ptf_config_file
  echo "    </periscope>" >> $ptf_config_file
